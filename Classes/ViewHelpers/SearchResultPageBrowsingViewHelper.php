...
/**
 * This is a full clone of the is:PageBrowsingViewHelper
 * search for sp@studioneun.de
 *
 ...
 * @internal
 */
class SearchResultPageBrowsingViewHelper extends AbstractViewHelper implements CompilableInterface
{
    /**
     * @var string
     */
...
//        // 2016-08-02 sp@studioneun.de: no previous page
//        if ($currentPage > 0) {
...
//        // 2016-08-02 sp@studioneun.de: custom html code
...
//                $content .= '<li>' . $label . '</li>';
//            }
            $label = self::makecurrentPageSelector_link($label, $a, $freeIndexUid, $currentPage);
            $content .= '<li>' . $label . '</li>';
        }
        // next link
//        // 2016-08-02 sp@studioneun.de: no next page
//        if ($currentPage < $pageCount - 1) {
//            $label = LocalizationUtility::translate('displayResults.next', 'IndexedSearch');
//            $content .= '<li class="page-next">' . self::makecurrentPageSelector_link($label, ($currentPage + 1), $freeIndexUid) . '</li>';
//        }

//        // 2016-08-02 sp@studioneun.de: list type ol instead of ul
//        return '<ul class="tx-indexedsearch-browsebox">' . $content . '</ul>';
        return '<ol class="flex-control-nav">' . $content . '</ol>';
    }
     ...
     * @param int $aP active page value
     * @return string Input string wrapped in <a> tag with onclick event attribute set.
     */
    protected static function makecurrentPageSelector_link($str, $p, $freeIndexUid, $aP = 0)
    {
        ...
        $active = ($p === $aP) ? ' class="flex-active"' : '';
        return '<a href="#" onclick="' . htmlspecialchars($onclick) . '"' . $active . '>' . $str . '</a>';
    }
}
